...
export function EducatorDashboard() {
  ...
  useEffect(() => {
    // For simplicity in this example, educators see all trips.
    // In a real app, you might filter trips by `trip.educatorId` or similar.
    setTrips(allTrips);
    if (allTrips.length > 0) {
      setActiveTripId(allTrips[0].id);
    }
  }, [user]);

  if (!user || user.role !== 'educator') {
    // This component is only for educators.
    return null;
  }
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
          <div className="md:col-span-1 space-y-4">
            {trips.map((trip, i) => (
              ...
              >
                <TripListItem trip={trip} isActive={activeTripId === trip.id} />
              </div>
            ))}
          </div>
...
function TripListItem({ trip, isActive }: { trip: Trip, isActive: boolean }) {
  return (
    <Card className={cn(
      "p-4 cursor-pointer transition-all duration-300 rounded-2xl border-2",
      isActive ? 'border-primary bg-primary/10' : 'border-border bg-card hover:bg-secondary'
    )}>
      <h3 className="font-light text-lg uppercase font-body tracking-wider">{trip.name}</h3>
