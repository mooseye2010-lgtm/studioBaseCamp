...
export function StudentDashboard() {
  ...
  useEffect(() => {
    if (user) {
      // Filter trips to show only those assigned to the current student.
      const assignedTrips = allTrips.filter(trip => trip.assignedStudentIds.includes(user.id));
      setTrips(assignedTrips);
      
      // If there's only one trip, navigate directly to its checklist.
  ...
  // This component is only for students.
  if (!user || user.role !== 'student') {
    return null;
  }
  
  // Display a message if no trips are assigned.
  if (trips.length === 0) {
    return <NoTripsState />;
  }
  
  // While redirecting for a single trip, show nothing.
  if (trips.length === 1) {
    return null;
  }
