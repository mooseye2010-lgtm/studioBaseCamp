...
export function Checklist({ tripId }: { tripId: string }) {
  ...
  const [progressSent, setProgressSent] = useState(false); // Tracks if the final "Progress Sent" message has been shown
  ...
  useEffect(() => {
    ...
    if (foundTrip && user) {
      ...
      // Initialize statuses from trip items, using saved progress if available.
  ...
  // Trigger the "rave" celebration when the checklist is 100% complete.
  useEffect(() => {
    if (completionPercentage === 100 && !progressSent) {
      setShowRave(true);
      const raveTimer = setTimeout(() => {
        setShowRave(false);
        setProgressSent(true); // Mark progress as sent to show the final message
      }, 5000); // Rave duration
      return () => clearTimeout(raveTimer);
    }
  ...
  const handleItemCheckedChange = (itemId: string, checked: boolean) => {
    setStatuses(prevStatuses =>
      prevStatuses.map(s => (s.itemId === itemId ? { ...s, completed: checked } : s))
    );
  ...
  if (!user || !trip) {
    return <p className="p-12 uppercase font-body tracking-wider">Loading checklist...</p>;
  }
...
function ChecklistHeader({ trip }: { trip: Trip }) {
    return (
            ...
            <Collapsible>
                <div className="p-4 flex items-center gap-5">
                    ...
                    <div className="flex-1 grid gap-1.5">
                        <Label htmlFor={`item-${item.id}`} className={cn(
                            "font-medium text-lg cursor-pointer transition-colors uppercase font-body tracking-wider",
                            status.completed && "line-through text-muted-foreground",
                            !allRequirementsMet && "cursor-not-allowed opacity-60"
                        )}>
                            {item.name}
                        </Label>
...
// A custom Checkbox component with added flair and tooltips.
function ItemCheckbox({ id, checked, onCheckedChange, disabled }: { id: string, checked: boolean, onCheckedChange: (checked: boolean) => void, disabled?: boolean }) {
    ...
    const tooltipText = disabled ? "Complete all sub-requirements first" : "Mark as complete";
...
// Displays the details for a packing item inside the collapsible content area.
function ItemDetails({ item, status, onRequirementCheckedChange }: PackingItemCardProps) {
  return (
